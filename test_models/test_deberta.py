...
def main():
    ...
    y_val = df_val['specialist_label'].tolist()
    
    print("Loading DeBERTa from models/deberta.joblib...")
    model = joblib.load('models/deberta.joblib')
    
    y_pred = model.predict(X_val)
    y_proba = model.predict_proba(X_val)
    
    multi_labels = {int(k): v for k, v in config['labels']['multiclass'].items()}
    metrics = compute_metrics(y_val, y_pred, y_proba, multi_labels)
    
    reporter = ModelReporter()
    reporter.report_model("DeBERTa", None, metrics)
