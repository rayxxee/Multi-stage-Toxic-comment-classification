...
def main():
    ...
    _, df_val = train_test_split(df, test_size=config['training']['test_size'], random_state=config['random_seed'], stratify=df['gatekeeper_label'])
    
    X_val = df_val['comment_text'].tolist()
    y_val = df_val['gatekeeper_label'].tolist()
    
    print("Loading FastText from models/fasttext.joblib...")
    model = joblib.load('models/fasttext.joblib')
    ...
    reporter.report_model("FastText", None, metrics)
